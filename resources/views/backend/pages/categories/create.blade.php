      ...
      <div class="card">
        ...
        <div class="card-body">
          <form action="{{ route('admin.category.store') }}" method="post"enctype="multipart/form-data">
            ...
            <div class="form-group">
              <label for="exampleInputPassword1">Parent Category(optinal)</label>
              <select class="form-control" name="parent_id">
                <option value="">Please select a Parent cateory</option>
                    <?php foreach ($main_categories as $category): ?>
                      <option value="{{$category->id}}">{{$category->name}}</optiion>
                    <?php endforeach; ?>
              </select>
            </div>
            <div class="form-group">
